...
$(function () {
    ...
    $("#ef_expire").datepicker({
        language: 'pt-BR',
        format: 'mm-dd-yyyy',

    });
    $("#lease_expire").datepicker({
        language: 'pt-BR',
        format: 'mm-dd-yyyy',
    });
    $("#elease_expire").datepicker({
        language: 'pt-BR',
        format: 'mm-dd-yyyy',
    });
...
function editRule(ruleId = null) {
    if (ruleId) {
        ...
        $.ajax({
            url: '../php_action/fetchSelectedRateRule.php',
            type: 'post',
            data: { id: ruleId },
            dataType: 'json',
            success: function (response) {
                ...
                $('#ef_24_36').val(response['f_24-36']);
                $('#ef_37_48').val(response['f_37-48']);
                $('#ef_49_60').val(response['f_49-60']);
                $('#ef_61_72').val(response['f_61-72']);

                $('#ef_659_610_24_36').val(response['f_659_610_24-36']);
                $('#ef_659_610_37_60').val(response['f_659_610_37-60']);
                $('#ef_659_610_61_72').val(response['f_659_610_61-72']);
                // $('#ef_expire').val(response.f_expire);
                var fexpd = moment(response.f_expire).format('MM-DD-YYYY');
                $('#ef_expire').datepicker('update', fexpd)

                $('#elease_660').val(response.lease_660);
                $('#elease_659_610').val(response.lease_659_610);
                $('#elease_one_pay_660').val(response.lease_one_pay_660);
                $('#elease_one_pay_659_610').val(response.lease_one_pay_659_610);
                // $('#elease_expire').val(response.lease_expire);
                var lexpd = moment(response.lease_expire).format('MM-DD-YYYY');
                $('#elease_expire').datepicker('update', lexpd)
    ...
    if (ruleId) {
        e1.fire({
            ...
            if (t.isConfirmed == true) {
                ...
                $.ajax({
                    url: '../php_action/removeRateRule.php',
                    type: 'post',
                    data: { id: ruleId },
                    dataType: 'json',
                    success: function (response) {
