...
$sql = "SELECT inventory.age , inventory.stockno , inventory.vin , inventory.model, inventory.year, inventory.make , inventory.color , 
inventory.mileage, inventory.lot , inventory.balance, inventory.retail, inventory.certified, 
inventory.stocktype , inventory.wholesale , inventory.id as invId , inventory.status as invStatus , inspections.* FROM inventory LEFT JOIN inspections ON inventory.id = inspections.inv_id WHERE inventory.lot != 'LBO' AND inventory.location = '$location' LIMIT 3";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_array()) {
        ...
        if ($balance == '' || $balance == null || $balance == '0' && $row[12] != 'NEW') {
            $Gone += 1;
            $_gone = "Gone";
        }
