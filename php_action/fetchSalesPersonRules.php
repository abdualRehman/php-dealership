...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_array()) {
        ...
        $vin_check = ($row[4] === 'N/A') ? "Disabled" : $row[4];
        $insurance = ($row[5] === 'N/A') ? "Disabled" : $row[5];
        $trade_title = ($row[6] === 'N/A') ? "Disabled" : $row[6];
        $registration = ($row[7] === 'N/A') ? "Disabled" : $row[7];
        $inspection = ($row[8] === 'N/A') ? "Disabled" : $row[8];
        $salesperson_status = ($row[9] === 'N/A') ? "Disabled" : $row[9];
        $paid = ($row[10] === 'N/A') ? "Disabled" : $row[10];
